...
from qos_anomaly.config import RANDOM_STATE, SAMPLE_LOGS_PATH
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=SAMPLE_LOGS_PATH)
    args = parser.parse_args()

    df = generate_logs(n_rows=args.n_rows, anomaly_ratio=args.anomaly_ratio, seed=args.seed)
    csv_path = save_dataset(df, args.output)
